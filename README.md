...
- **[node9](https://github.com/node9-ai/node9-proxy)** 🟢 — Security and governance layer for AI coding agents and MCP servers (Claude Code, Codex CLI, Gemini CLI, Cursor, Copilot CLI and more) that checks every tool call before it runs: a credential jail, secret and PII detection, rules for destructive git, SQL and shell commands, per-service shields, human approval for held actions, MCP tool pinning, an egress allowlist and container sandbox, plus machine posture scoring, repo scanning for hijackable agent workflows, and one audit record across agents. *(Node9)* — **note:** enforcement runs locally with no account; a hosted team workspace and a paid Pro tier are optional. `node9 init` asks before sending a one-time install ping. The egress allowlist is opt-in. *(★ 216 · updated 2026-09-25)*
